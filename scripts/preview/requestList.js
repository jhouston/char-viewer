...
/** @lends RequestList */
{
    tableTag:
        TABLE({"class": "netTable", cellpadding: 0, cellspacing: 0, onclick: "$onClick",
            ...
            TBODY({"class" : "requestListContainer"},
    ...
    fileTag:
        FOR("entry", "$files",
            // Couldn't get around the offset error in the IF tag update. Use this trick instead
            FOR("networkEntry", "$entry|asNetworkEntryArray", 
                TR({"class": "netRow networkEntryRow loaded $networkEntry|getRequestId",
                    $isExpandable: "$networkEntry|isExpandable",
                    $responseError: "$networkEntry|isError",
                    $responseException: "$networkEntry|isException",
                    $responseInFlight: "$networkEntry|isInFlight",
                    $responseRedirect: "$networkEntry|isRedirect",
                    $fromCache: "$networkEntry|isFromCache",
                    onmouseover : "$onMouseOver",
                    onmouseout : "$onMouseOut"},
                    TD({"class": "netHrefCol netCol"},
                        DIV({"class": "netHrefLabel netLabel",
                             style: "margin-left: $networkEntry|getIndent\\px"},
                            "$networkEntry|getHref"
                        ),
                        DIV({"class": "netFullHrefLabel netHrefLabel netLabel",
                             style: "margin-left: $networkEntry|getIndent\\px"},
                            "$networkEntry|getFullHref"
                        )
                    ),
                    TD({"class": "netStatusCol netCol"},
                        DIV({"class": "netStatusLabel netLabel"}, "$networkEntry|getStatus")
                    ),
                    TD({"class": "netTypeCol netCol"},
                        DIV({"class": "netTypeLabel netLabel"}, "$networkEntry|getType")
                    ),
                    TD({"class": "netDomainCol netCol"},
                        DIV({"class": "netDomainLabel netLabel"}, "$networkEntry|getDomain")
                    ),
                    TD({"class": "netSizeCol netCol"},
                        DIV({"class": "netSizeLabel netLabel"}, "$networkEntry|getSize")
                    ),
                    TD({"class": "netTimeCol netCol"},
                        DIV({"class": "netTimelineBar"},
                            ...
                            DIV({"class": "netReceivingBar netBar"},
                                SPAN({"class": "netTimeLabel"}, "$networkEntry|getElapsedTime")
                            )
            ...
            FOR("appStateEntry", "$entry|asAppStateEntryArray", 
                TR({"class": "netRow appStateEntryRow loaded",
                    onmouseover : "$onMouseOver",
                    onmouseout : "$onMouseOut"},
                    TD({"class": "netHrefCol netCol"},
                        DIV({"class": "netHrefLabel netLabel",
                             style: "margin-left: $appStateEntry|getIndent\\px"},
                            "$appStateEntry|getShortEntryName"
                        ),
                        DIV({"class": "netFullHrefLabel netHrefLabel netLabel",
                             style: "margin-left: $appStateEntry|getIndent\\px"},
                            "$appStateEntry|getFullEntryName"
                        )
                    ),
                    TD({"class": "netStatusCol netCol"},
                        DIV({"class": "netStatusLabel netLabel"}, "$appStateEntry|getAppState")
                    ),
                    TD({"class": "netTypeCol netCol"},
                        DIV({"class": "netTypeLabel netLabel"}, "")
                    ),
                    TD({"class": "netDomainCol netCol"},
                        DIV({"class": "netDomainLabel netLabel"}, "$appStateEntry|getEntryPackage")
                    ),
                    TD({"class": "netSizeCol netCol"},
                        DIV({"class": "appStateSizeFiller netLabel"}, "")
                    ),
                    TD({"class": "netTimeCol netCol"},
                        DIV({"class": "netTimelineBar"},
                            "&nbsp;",
                            DIV({"class": "appStateBar $appStateEntry|getAppStateBarClass netBar"})
                        )
                    ),
                    TD({"class": "netOptionsCol netCol"},
                        DIV({"class": "netOptionsLabel netLabel", onclick: "$onOpenOptions"})
    ...
    asAppStateEntryArray: function(entry)
    {
        var outArray = [];
        if (this.isAppStateEntry(entry)) {
            outArray = [ entry ];
        }
        return outArray;
    },    
       
    asNetworkEntryArray: function(entry)
    {
        var outArray = [];
        if (this.isNetworkEntry(entry)) {
            outArray = [ entry ];
        }
        return outArray;
    },
    ...
    getAppState(entry)
    {
        return entry.appState.toUpperCase();
    },
    
    getAppStateBarClass(entry)
    {
        var className;
        if (entry.appStateType.toLowerCase() == "active")
        {
            className = "appStateBarActive";
        }
        else
        {
            className = "appStateBarInactive";
        }
        return className;
    },
    
    getEntryPackage(entry)
    {
        var entryName = entry.sourceName;
        var lastDot = entryName.lastIndexOf(".");
        if (lastDot == -1) {
            return entryName;
        }
        return entryName.substr(0, lastDot);
    },

    getIndent: function(entry)
    {
        return 0;
    },

    getFullEntryName(entry)
    {
        return entry.sourceName;
    },
    
    getShortEntryName(entry) 
    {
        var entryName = entry.sourceName;
        var lastDot = entryName.lastIndexOf(".");
        if (lastDot == -1) {
        ...
        return entryName.substr(lastDot+1);
    },

    isError: function(entry)
    {
        var errorRange = Math.floor(entry.response.status/100);
        return errorRange == 4 || errorRange == 5 || errorRange == 0;
    },
    
    isException: function(entry)
    {
        return (entry.response.status == 0) || (!!entry.response.exception);
    },
    
    isInFlight: function(entry)
    {
        return entry.response.status == -1;
    },
    
    isAppStateEntry: function(entry)
    {
        return entry.entryType == 'appState';
    },
    
    isNetworkEntry: function(entry)
    {
        return !entry.entryType || (entry.entryType == 'network');
    },
    
    isRedirect: function(entry)
    {
        // xxxHonza: 304?
        //var errorRange = Math.floor(entry.response.status/100);
        //return errorRange == 3;
    ...
    isFromCache: function(entry)
    {
        return entry.cache && entry.cache.afterRequest;
    },

    getHref: function(entry)
    {
        var entryName = Lib.getFileName(this.getFullHref(entry));
        return unescape(entry.request.method + " " + entryName);
    },

    getFullHref: function(entry)
    {
        return unescape(entry.request.url);
    },
    
    getRequestId: function(entry)
    {
        return "reqId_" + entry.requestId;
    },
    ...
    getDomain: function(entry)
    {
        return Lib.getPrettyDomain(entry.request.url);
    },

    getSize: function(entry)
    {
        var resBodySize = entry.response.bodySize;
        var resHeaderSize = entry.response.headersSize;
        var reqBodySize = entry.request.bodySize;
        var reqHeaderSize = entry.request.headersSize;
                
        var size = 0;
        if (resBodySize && resBodySize != -1) {
            size = size + resBodySize;
        }
        if (resHeaderSize && resHeaderSize != -1) {
            size = size + resHeaderSize;
        }
        if (reqBodySize && reqBodySize != -1) {
            size = size + reqBodySize;
        }
        if (reqHeaderSize && reqHeaderSize != -1) {
            size = size + reqHeaderSize;
        }

        return this.formatSize(size);
    },

    isExpandable: function(entry)
    {
        var hasHeaders = entry.response.headers.length > 0;
        var hasDataURL = entry.request.url.indexOf("data:") == 0;
    ...
    getElapsedTime: function(entry)
    {
        // Total request time doesn't include the time spent in queue.
        //var elapsed = entry.time - entry.timings.blocked;
        var time = Math.round(entry.time * 10) / 10;
    ...
    onMouseOver: function(event) {
        this.highlightRows(event, true);
    },
    
    onMouseOut: function(event) {
        this.highlightRows(event, false);
    },
    
    highlightRows: function(event, highlight) {
        if(!event.target) {
            return;
        }
        
        var tableRow = Lib.getAncestorByClass(event.target, "netRow");
        if(!tableRow) {
            return;
        }
        
        var requestId = this.getElementRequestId(tableRow);
        if(!requestId) {
            return;
        }
        
        var tableBody = Lib.getAncestorByClass(tableRow, "requestListContainer");
        if(!tableBody) {
            return;
        }
        
        var rows = tableBody.querySelectorAll("." + requestId);
        var color = highlight ? "#E1ECFF" : "";
        for(var i=0; i<rows.length; i++) {
            rows[i].style.backgroundColor = color;
        }
    }, 
    
    getElementRequestId: function(element) {
        if(!element) {
            return null;
        }
        ...
        var allClasses = classString.split(" ");
        for(var i=0; i<allClasses.length; i++) {
            if(allClasses[i].indexOf('reqId_') == 0) {
                return allClasses[i];
            }
        }
    ...
    updateTimeline: function(page)
    {
        ...
        for (var row = this.firstRow; row; row = row.nextSibling)
        {
            ...
            if (this.isNetworkEntry(file)) 
            {
                this.updateNetworkEntryTimeline(row, page, file, phase);
            }
            else if (this.isAppStateEntry(file))
            {
                this.updateAppStateEntryTimeline(row, page, file, phase);
            }
        }
    },
    
    updateAppStateEntryTimeline : function(row, page, file, phase) {
        ...
        var appStateBar = timelineBar.children[0];
        
        appStateBar.style.left = this.barOffset + "%";
    },
    
    updateNetworkEntryTimeline : function(row, page, file, phase) {
        phase = this.calculateFileTimes(page, file, phase);
    ...
    showInfoTip: function(infoTip, target, x, y)
    {
        ...
        if (row)
        {
            if (Lib.getAncestorByClass(target, "appStateEntryRow"))
            {
                // Nothing to do yet. Pop an info bar if we want to sometime... 
            }
            else if (Lib.getAncestorByClass(target, "netBar"))
            {
...
{
    tag:
        DIV({"class": "sizeInfoTip"}, "$entry|getSize"),

    zippedTag:
        DIV(
            DIV({"class": "sizeInfoTip"}, "$entry|getBodySize"),
            DIV({"class": "sizeInfoTip"}, "$entry|getContentSize")
        ),
    ...
    sizeRowTag:
        TR(
            TD({"class": "sizeInfoTip"}, "$sizeLabel"),
            TD({"class": "sizeInfoTip"}, "$sizeValue")
        ),
    ...
    getSize: function(entry)
    {
        ...
        return this.formatSize(bodySize);
    },
    ...
    formatSize: function(size)
    {
        return Lib.formatString(Strings.tooltipSize,
            Lib.formatSize(size),
            Lib.formatNumber(size));
    },

    getBodySize: function(entry)
    {
        var bodySize = entry.response.bodySize;
        if (bodySize < 0)
            return Strings.unknownSize;
...
//        return this.zippedTag.replace({file: file}, parentNode);
        ...
        var input = requestList.input;
        var file = row.repObject;
        var infoTip = EntrySizeInfoTip.tableTag.replace({}, parentNode);
        
        this.sizeRowTag.insertRows({
            sizeLabel: "Request Body Size",
            sizeValue: this.formatSize(file.request.bodySize)
        }, infoTip.firstChild);
        
        this.sizeRowTag.insertRows({
            sizeLabel: "Request Headers Size",
            sizeValue: this.formatSize(file.request.headersSize)
        }, infoTip.firstChild);
        
        this.sizeRowTag.insertRows({
            sizeLabel: "Response Body Size",
            sizeValue: this.formatSize(file.response.bodySize)
        }, infoTip.firstChild);
        
        this.sizeRowTag.insertRows({
            sizeLabel: "Response Headers Size",
            sizeValue: this.formatSize(file.response.headersSize)
        }, infoTip.firstChild);
    }
});
